...
import { getPhaseLabel } from '../../domain/types/ranking';
...
export function RankingPreviewCard({ type }: RankingPreviewCardProps) {
  ...
  const { user } = useAuth();
  const { phase, general, loading, selectedPhase } = useRankings(user?.id ?? null);

  const isPhase = type === 'phase';
  const data = isPhase ? phase : general;
  const title = isPhase ? 'Ranking por Fase' : 'Ranking General';
  const subtitle = isPhase && selectedPhase ? getPhaseLabel(selectedPhase) : 'Tabla de posiciones';
  ...
  return (
    <Card onClick={() => navigate('/ranking')} className="cursor-pointer">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isPhase ? 'bg-white/8 text-textMuted' : 'bg-cupYellow/10 text-cupYellow'}`}>
          {isPhase ? (
